    ...
    private void rentalProcess(int movieNumber) {
        ...
        if(users.size() > 0) {
            ...
            if (userNums.contains(userNumber)) {
                ...
                rentalMovie.setRentalUser(rentalUser);//랜탈유저등록
                rentalUser.setTotalPaying(rentalMovie.getCharge());//대여금액을 회원 총 결제 금액에 누적 갱신


                String phoneNumer = rentalUser.getPhoneNumber();
                //새로운 주문생성
                Order newoder = new Order(rentalUser,rentalMovie);
                rentalUser.addOrder(newoder);
                //lastIndex 해당 문자열의 뒤에서탐색
                //뒤에서 탐색 해서 - 을 찯아라 그 - 이후러부터 끝까지 추출
                System.out.printf("%s(%s)회원 처리완료\n",rentalUser.getUserName(),rentalUser.getPhoneNumber().substring(phoneNumer.lastIndexOf("-")));


                System.out.printf("현재등급 %s 청누적 급액 %d원\n ",rentalUser.getGrade(),rentalUser.getTotalPaying());
